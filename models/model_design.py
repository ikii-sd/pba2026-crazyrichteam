...
class TextCNNMultiOutput(nn.Module):
    """
    Multi-output TextCNN:
    Embedding -> Conv blocks -> Global max pooling -> Shared Dense -> 2 Heads
    """
    ...
    def __init__(
        ...
        self.dropout = nn.Dropout(dropout)
        self.shared_fc = nn.Linear(num_filters * len(kernel_sizes), num_filters)
        self.relu = nn.ReLU()

        self.sentiment_head = nn.Linear(num_filters, num_sentiment_classes)
        self.emotion_head = nn.Linear(num_filters, num_emotion_classes)
...
if __name__ == "__main__":
    ...
    textcnn = TextCNNMultiOutput()
    textcnn_params = count_parameters(textcnn)
    sent_logits, emo_logits = textcnn(dummy_input)

    print(f"TextCNN parameters: {textcnn_params:,}")
    print(f"TextCNN sentiment logits shape: {tuple(sent_logits.shape)}")
    print(f"TextCNN emotion logits shape: {tuple(emo_logits.shape)}")
